...
def find_kots_params(n, secpar, rho, alpha_w, epsilon, verbose, RHF_KOTS):
  ...
  phi = 1
  while sis_is_hard:
    # We have a circular dependency between gamma, beta_sigma, and q.
    ...
    sis_is_hard, rhf = msis_estimation(q, n, 1, gamma, beta_kots, RHF_KOTS)
    if sis_is_hard:
...
def find_hvc_params(n, secpar, rho, tau, alpha_w, xi, qprime, epsilon, verbose, RHF_HVC):
  ...
  while sis_is_hard:
    ...
    # We have a circular dependency between kappa, beta_agg, and q. To solve
    ...
    (path_sis_is_hard, rhf1) =  msis_estimation(q, n, 1, 2 * kappa, beta_hvc, RHF_HVC)
    (payload_sis_is_hard, rhf2) =  msis_estimation(q, n, 1, xi*kappaprime, beta_hvc, RHF_HVC)
...
def find_param(n, secpar, rho, tau, epsilon, verbose, RHF_KOTS, RHF_HVC):
  """
  ...
  # Compute chi according to constraint 10
  ...
  kots_param = find_kots_params(n, secpar, rho, alpha_w, epsilon, verbose, RHF_KOTS)
  ...
  hvc_param = find_hvc_params(n, secpar, rho, tau, alpha_w, 2, kots_param["q'"], epsilon, verbose, RHF_HVC)
  return (alpha_w,chi,kots_param,hvc_param)

def find_params(n,secpars,taus,rhos,epsilons,verbosity, RHF_KOTS_BY_RHO, RHF_HVC):
  """
  ...
  for secpar in secpars:
    ...
    for tau in taus:
      params[secpar][tau] = {}
      for rho in rhos:
        RHF_KOTS = RHF_KOTS_BY_RHO[rho]
        params[secpar][tau][rho] = {}
        for epsilon in epsilons:
         params[secpar][tau][rho][epsilon] = find_param(n, secpar, rho, tau, epsilon, bool(verbosity>1), RHF_KOTS, RHF_HVC)
         to_tabulate.append([
           str(secpar),
           str(tau),
           str(rho),
           ...
           str(round_half_up((params[secpar][tau][rho][epsilon][2]["size"])/8/1024))+" KB",
           str(round_half_up((params[secpar][tau][rho][epsilon][3]["size"])/8/1024))+" KB",
           str(round_half_up((params[secpar][tau][rho][epsilon][2]["size"]+params[secpar][tau][rho][epsilon][3]["size"]+ceil(log(params[secpar][tau][rho][epsilon][1],2)))/8/1024))+" KB"
         ])
  
  if verbosity > 0:
    with open("chipmunk_summary.txt", "w") as f:
...
n = 1024
# number of users
rhos = [1024, 32768, 131072, 1048576]
# height of the tree
taus = [12, 16, 20, 24]
...
params = find_params(n,secpars,taus,rhos,epsilons,verbosity, RHF_KOTS_BY_RHO, RHF_HVC)
